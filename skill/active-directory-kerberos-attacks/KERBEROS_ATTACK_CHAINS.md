...
- [1. CHAIN: KERBEROAST → CONSTRAINED DELEGATION → DOMAIN ADMIN](#1-chain-kerberoast-constrained-delegation-domain-admin)
- [2. CHAIN: RBCD + KERBEROS → LATERAL MOVEMENT](#2-chain-rbcd-kerberos-lateral-movement)
- [3. CHAIN: UNCONSTRAINED DELEGATION + PRINTERBUG → DCSYNC](#3-chain-unconstrained-delegation-printerbug-dcsync)
- [4. CHAIN: AS-REP ROAST → ACL ABUSE → DCSYNC](#4-chain-as-rep-roast-acl-abuse-dcsync)
- [5. CHAIN: TARGETED KERBEROAST VIA ACL](#5-chain-targeted-kerberoast-via-acl)
- [6. CHAIN: GOLDEN TICKET → CROSS-DOMAIN ESCALATION](#6-chain-golden-ticket-cross-domain-escalation)
- [7. CHAIN: SHADOW CREDENTIALS + KERBEROS](#7-chain-shadow-credentials-kerberos)
- [8. ATTACK CHAIN SELECTION GUIDE](#8-attack-chain-selection-guide)
<!-- zhiyugo:toc:end -->

> **Technical reference scope**: Load this for end-to-end Kerberos attack chains that combine multiple AD techniques. Assumes the main [SKILL.md](./SKILL.md) is already loaded for individual Kerberos attacks. Use when planning multi-step attack paths from initial foothold to domain admin.
