...
- [1. H2.CL — HTTP/2 Content-Length Desync](#1-h2cl-http2-content-length-desync)
- [2. H2.TE — HTTP/2 Transfer-Encoding Desync](#2-h2te-http2-transfer-encoding-desync)
- [3. CL.0 — CONNECTION CLOSE DESYNC](#3-cl0-connection-close-desync)
- [4. FAT GET REQUEST SMUGGLING](#4-fat-get-request-smuggling)
- [5. REQUEST SMUGGLING → CACHE POISONING CHAIN](#5-request-smuggling-cache-poisoning-chain)
- [6. CLIENT-SIDE DESYNC (CSD)](#6-client-side-desync-csd)
- [7. CDN / REVERSE PROXY BEHAVIOR MATRIX](#7-cdn-reverse-proxy-behavior-matrix)
- [8. TESTING METHODOLOGY](#8-testing-methodology)
<!-- zhiyugo:toc:end -->

> **Technical reference scope**: Load this when you need H2.CL/H2.TE byte-level payloads, CL.0 desync, Fat GET smuggling, smuggling→cache poisoning chains, client-side desync (CSD) flows, or CDN/reverse-proxy behavior matrices. Assumes the main [SKILL.md](./SKILL.md) is already loaded for CL.TE, TE.CL, TE.TE fundamentals.
