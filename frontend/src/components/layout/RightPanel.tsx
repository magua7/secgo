  ...
  return <aside className="right-panel">
    ...
    <div className="right-tabs-row">
      <div className="right-tabs" role="tablist">
        ...
        <button className={tab === 'decisions' ? 'active' : ''} onClick={() => onTabChange('decisions')}>决策理由</button>
    ...
    <div className="right-content">
      {empty ? <PanelEmptyState title="尚未开始任务" detail="提交安全任务后，这里将展示 Agent 执行轨迹、工具调用与阶段变化。" /> : direct ? <PanelEmptyState title="本轮为直接回复" detail="未触发 Agent 执行或工具调用。" /> : <>
        ...
        {tab === 'decisions' && <div className="decision-list">{view.decisions.length ? view.decisions.map((item) => <article key={item.id} className="decision-card"><div className="decision-header"><strong className={`decision-trigger decision-trigger--${item.trigger}`}>{item.trigger}</strong><small>{item.timestamp ? new Date(item.timestamp * 1000).toLocaleTimeString('zh-CN', { hour12: false }) : ''}</small></div><p className="decision-detail">{item.trigger_detail}</p><div className="decision-reason"><strong>选择理由：</strong><span>{item.reason}</span></div><details className="decision-candidates"><summary>候选策略（{item.candidates.length} 个）</summary>{item.candidates.map((c, i) => <div key={c.id} className={`decision-candidate ${c.id === item.selected ? 'selected' : 'rejected'}`}><span className="candidate-label">{c.id === item.selected ? '✅ 选中' : '❌ 放弃'}</span><strong>{c.description}</strong><small>风险: {c.risk} | 预期: {c.expected_outcome}</small></div>)}</details></article>) : <PanelEmptyState title="暂无决策记录" detail="尚未产生需重规划的决策（连续失败/重复调用/无进展时自动触发）。" />}</div>}
