  ...
  return <aside className="right-panel">
    ...
    <div className="right-content">
      {empty ? <PanelEmptyState title="尚未开始任务" detail="提交安全任务后，这里将展示 Agent 执行轨迹、工具调用与阶段变化。" /> : direct ? <PanelEmptyState title="本轮为直接回复" detail="未触发 Agent 执行或工具调用。" /> : <>
        ...
        {tab === 'evidence' && <div className="evidence-list">{view.evidence.length ? view.evidence.map((item) => <article key={item.id ?? `${item.source}-${item.summary}`}><strong>{item.title ?? item.source}</strong><DetailText detail={item.summary} /></article>) : <PanelEmptyState title="本轮暂未形成可确认的关键证据" detail="执行轨迹中的工具结果尚未达到关键证据标准；出现 Flag、漏洞验证成功等确认性发现后会自动展示在这里。" />}</div>}
