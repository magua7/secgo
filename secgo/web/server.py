...
def _ensure_started() -> None:
    ...
    for event_name in (
        ...
        "decision:reason",
        # 附件图片视觉分析结果：前端据此在执行轨迹中展示「系统真的看了图」
        "attachment:analyzed",
    ):
...
async def api_chat(request: Request, _auth=Depends(require_ready_state)) -> JSONResponse:
    ...
    # 把图片视觉分析结果作为执行轨迹事件推送（明确带 turn_id，供前端路由到正确 Turn）。
    for attachment_id, analysis in attachment_analyses.items():
        event_bus.emit("attachment:analyzed", {
            "session_id": session_id,
            "turn_id": turn_id,
            ...
            "security_findings": analysis.get("security_findings") or [],
            "scene_tags": analysis.get("scene_tags") or [],
